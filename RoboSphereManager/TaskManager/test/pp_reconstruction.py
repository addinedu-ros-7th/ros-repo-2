...
class DynamicWaypointNavigator(Node):
    ...
    def handle_task_request(self, request, response):
        ...
        try:
            ...
            self.waypoints = self.generate_waypoints(self.target) #jh0211
            ...
            self.last_ping_pong_time = time.time()  #  마지막으로 탁구공이 감지된 시간 #jh0210
            self.max_wait_time = 5.0  #  탁구공 감지가 없으면 5초 후 원래 웨이포인트로 이동 #jh0210
